...
export interface BatchProgress {
  id: string
  type: 'translate' | 'tts' | 'vocal_separation' | 'asr' | 'speaker_diarization' | 'speaker_diarization'
...
export interface BatchProgressState {
  ...
  asr: BatchProgress
  speaker_diarization: BatchProgress
}
...
export function useBatchProgress() {
  ...
  const progressState = reactive<BatchProgressState>({
    ...
    speaker_diarization: {
      ...
      status: 'idle',
      total: 100, // 说话人识别使用百分比进度
      completed: 0,
      failed: 0,
      errorMessages: [],
      canCancel: true,  // 说话人识别可以取消
      canPause: false   // 说话人识别不可暂停
    }
  })
  ...
  // 完成批量操作
  const completeBatchOperation = (type: 'translate' | 'tts' | 'vocal_separation' | 'asr' | 'speaker_diarization') => {
    const progress = progressState[type]
  ...
  // 取消批量操作
  const cancelBatchOperation = (type: 'translate' | 'tts' | 'vocal_separation' | 'asr' | 'speaker_diarization') => {
    const progress = progressState[type]
  ...
  // 暂停/恢复批量操作
  const togglePauseBatchOperation = (type: 'translate' | 'tts' | 'vocal_separation' | 'asr' | 'speaker_diarization') => {
    const progress = progressState[type]
  ...
  // 设置错误状态
  const setErrorState = (type: 'translate' | 'tts' | 'vocal_separation' | 'asr' | 'speaker_diarization', error: string) => {
    const progress = progressState[type]
    console.log(`[BatchProgress] Setting error state for ${type}:`, error)
    console.log(`[BatchProgress] Before - status: ${progress.status}, errorMessages:`, progress.errorMessages)

    progress.status = 'error'
    progress.errorMessages.push(error)
    progress.currentItem = undefined

    console.log(`[BatchProgress] After - status: ${progress.status}, errorMessages:`, progress.errorMessages)
    console.error(`[BatchProgress] Error in ${type} operation:`, error)
  }

  // 关闭进度显示（手动隐藏已完成的任务）
  const dismissProgress = (type: 'translate' | 'tts' | 'vocal_separation' | 'asr' | 'speaker_diarization') => {
    const progress = progressState[type]
  ...
  // 重置进度
  const resetProgress = (type?: 'translate' | 'tts' | 'vocal_separation' | 'asr' | 'speaker_diarization') => {
  ...
  // 获取进度百分比
  const getProgressPercentage = (type: 'translate' | 'tts' | 'vocal_separation' | 'asr' | 'speaker_diarization') => {
    const progress = progressState[type]
  ...
  // 获取状态显示文本
  const getStatusText = (type: 'translate' | 'tts' | 'vocal_separation' | 'asr' | 'speaker_diarization') => {
    const progress = progressState[type]
    let typeName = '操作'
    ...
    else if (type === 'speaker_diarization') typeName = '说话人识别'
